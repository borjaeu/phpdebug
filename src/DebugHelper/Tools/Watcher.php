...
class Watcher extends Abstracted
{
    ...
    public function watch( $silent = false, $trace_file = false, $ignore = false )
    {
        ...
        $this->trace_file =  \DebugHelper::getDebugDir(). $trace_key;
        if ($trace_file && is_file( $this->trace_file )) {
            return;
        }

        file_put_contents( $this->trace_file . '.svr', print_r( $_SERVER, true ) );
        ...
        register_shutdown_function( '\DebugHelper\Tools\Watcher::endWatch' );
    }
    ...
    {
        if (empty( $this->trace_file )) {
            return;
        }
        xdebug_stop_trace();

        $coverage = $this->getCodeCoverage( $this->trace_file . '.raw' );
